param(
    [ValidateSet("Validate", "DeepValidate", "Lock", "PrepareFeatures", "Smoke", "Matrix", "PointRJSD", "FisherSupervised", "Status", "Summarize", "All")]
    [string]$Stage = "Status",
    [string]$CondaEnv = "cmrd",
    [string]$RunRoot = "runs/faced_relative_supervised_monitor_seed42",
    [int[]]$Folds = @(),
    [switch]$Resume,
    [switch]$RetryFailed,
    [switch]$AllowPartial
...
$Config = "configs/faced/relative_supervised_monitor.yaml"
$Runner = "scripts/run_faced_relative_supervised.py"
...
switch ($Stage) {
    ...
    "Matrix" { Invoke-Matrix @("frequency_point_rjsd_base", "fisher_rao_supervised_lda2_base") }
    "PointRJSD" { Invoke-Matrix @("frequency_point_rjsd_base") }
    "FisherSupervised" { Invoke-Matrix @("fisher_rao_supervised_lda2_base") }
    "Status" { Invoke-Status }
    "Summarize" { Invoke-Summarize }
    "All" {
        Invoke-Validate
        Invoke-Lock
        Invoke-Matrix @("frequency_point_rjsd_base", "fisher_rao_supervised_lda2_base")
        Invoke-Summarize
    }
}
